...
export class ExampleFindSpaceService {
  ...
  public checker(ask: Ask) {
    const serviceProviderData: ServiceProviderData = {
      ...
      items: [
        {
          item: utils.arrayify(
            utils.formatBytes32String('UNIQUE-CODE-FOR-SPACE-1')
          ),
          payload: Item.toBinary({
            ...
            payload: Space.toBinary({
              ...
              privateHomeOneof: {
                oneofKind: 'privateHome',
                privateHome: false
              },
              suiteOneof: { oneofKind: 'suite', suite: false },
              tierOneof: {
                oneofKind: 'tier',
                tier: SpaceTier.DEFAULT_STANDARD
              },
              views: {
                viewOfValleyOneof: {
        ...
        {
          item: utils.arrayify(
            utils.formatBytes32String('UNIQUE-CODE-FOR-SPACE-2')
          ),
          payload: Item.toBinary({
            name: 'Fancy suite',
            description: 'Now with an onsen!',
        ...
        {
          item: utils.arrayify(
            utils.formatBytes32String('UNIQUE-CODE-FOR-BREAKFAST')
          ),
          payload: Item.toBinary({
  ...
  findSpaces(spaces, ask) {
    ...
    spaces.forEach((i) => {
      ...
      //check capacity
      if (
        !ExampleFindSpaceService.checkSuitableQuantity(
          numOfAdults,
          numOfChildren,
          ask.numPaxAdult,
          ask.numPaxChild
      ...
      if (
        ExampleFindSpaceService.checkAvailableDates(
          i.id,
          ask.checkIn,
          ask.checkOut
        )
